...
public:
    Skill() : m_name(""), m_speed(0), m_chance(0.0f), m_hp(0.0f), m_duration(0.0f), m_isActive(false) {}
    Skill(const std::string& name, int speed, float chance, float hp, float duration)
        : m_name(name), m_speed(speed), m_chance(chance), m_hp(hp), m_duration(duration),
    ...
    bool ShouldActivate(float raceProgress, float currentHp, float maxHp, bool isLeading) const;
};