...
void Horse::HorseTick(int leader_X, float deltaTime, const Map& map)
{
	...
	switch (m_type) {
	case PACESETTER: speedModType = 4; staminaDrain = 0.1f; break;
	case LEADER:     speedModType = 3; staminaDrain = 0.08f; break;
	case STALKER:
		speedModType = (distanceFromLeader >= 200) ? 3 : 1;
		staminaDrain = (distanceFromLeader >= 200) ? 0.08f : 0.04f;
		break;
	case CLOSER:
		speedModType = (leader_X >= map.GetFinishLine() * 2 / 3) ? 8 : -1;
...
void Horse::PrintPlayerList(const std::vector<Horse>& PlayerList)
{
	...
	for (size_t i = 0; i < PlayerList.size(); ++i)
	{
		...
		for (int j = 1; j < m_skillList.size(); ++j)
		{
